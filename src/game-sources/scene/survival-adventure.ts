...
import survivaladventuremap from "../map-json/suriviaval-adventure-map.json";
...
export default class SurvivalAdvantureScene extends Phaser.Scene {
  ...
  preload() {
    this.load.tilemapTiledJSON("survivaladventuremap", survivaladventuremap);
    ...
    this.load.image("Chest", chesttileImage);
  ...
  private setUpMapAndTileSet() {
    this.map = this.make.tilemap({ key: "survivaladventuremap" });
    ...
    const chestTile = this.map.addTilesetImage("Chest", "Chest");
    const oraktreeTile = this.map.addTilesetImage("Oak_Tree", "Oak_Tree");
    ...
    this.stuffLayer = this.map.createLayer(
      "stuff",
      [outdoordecorTile!, bridgeTile!, oraktreeTile!, chestTile!],
      0,
      0
    )!;
  }

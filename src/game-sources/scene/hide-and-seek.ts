...
import hideandseek from "../map-json/hide-and-seek-map.json";
...
export default class HideAndSeekScene extends Phaser.Scene {
  ...
  preload() {
    this.load.tilemapTiledJSON("hideandseekmap", hideandseek);
  ...
  private setUpMapAndTileSet() {
    this.map = this.make.tilemap({ key: "hideandseekmap" });
    ...
    this.map.createLayer("raod", pathTile!, 0, 0);
  }
